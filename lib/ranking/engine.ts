/**
 * Ranking engine ‚Ä‚Äù aggregates individual signals into a deterministic 0‚Ä‚Äú100 score.
 *
 * Weights (sum to 1.0):
 *   titleRelevance  0.30  ‚Ä‚Äù most important: is this even a PM role?
 *   skillsOverlap   0.20  ‚Ä‚Äù do skills from profile/JD match?
 *   domainFit       0.15  ‚Ä‚Äù does the domain match user's background?
 *   recency         0.15  ‚Ä‚Äù newer is better
 *   locationFit     0.10  ‚Ä‚Äù geography alignment
 *   workModelFit    0.10  ‚Ä‚Äù remote/hybrid/onsite preference
 *
...
export function scoreJob(job: RawJob, profile: UserProfile): ScoredJob {
  ...
  // Apply mismatch multiplier and scale to 0‚Ä‚Äú100
  const finalScore = Math.round(rawScore * penalty.multiplier * 100);
  const clampedScore = Math.max(0, Math.min(100, finalScore));

  // Build reasons list (3‚Ä‚Äú5 most informative items)
...
/**
 * Score a full FitResult (with all signal values) ‚Ä‚Äù used for API/debug responses.
 */
