/**
 * Ingestion pipeline — orchestrates all source connectors.
 *
 * Pipeline steps:
...
export async function runIngestionPipeline(
  ...
  try {
    // ── Step 1: Run all connectors in parallel ─────────────────────────────
    ...
    // ── Step 3: Load user profile for scoring ─────────────────────────────
    const profile = await getProfile();

    // ── Step 4: Score + upsert each job ───────────────────────────────────
    ...
    // ── Step 5: Mark jobs not seen in this run ────────────────────────────
