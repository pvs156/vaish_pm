/**
 * Ashby connector ‚Ä‚Äù queries public Ashby GraphQL job boards for a curated list of companies.
 *
      ...
      companies.map(async ({ slug, name }) => {
        ...
        try {
          ...
          errors.push({
            source: this.sourceId,
            stage: "parse",
            message: `Ashby ${slug}: schema parse failed ‚Ä‚Äù ${err instanceof Error ? err.message : String(err)}`,
          });
          return;
        }
