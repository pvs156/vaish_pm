/**
 * Greenhouse connector ‚Ä‚Äù queries public Greenhouse job boards for a curated list of companies.
 *
...
// Default curated list ‚Ä‚Äù expandable via GREENHOUSE_COMPANIES env var (comma-separated tokens)
      ...
      companies.map(async ({ token, name }) => {
        ...
        try {
          ...
          errors.push({
            source: this.sourceId,
            stage: "parse",
            message: `Greenhouse ${token}: schema parse failed ‚Ä‚Äù ${err instanceof Error ? err.message : String(err)}`,
          });
          return;
        }
