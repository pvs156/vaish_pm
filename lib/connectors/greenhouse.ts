/**
 * Greenhouse connector â” queries public Greenhouse job boards for a curated list of companies.
 *
      ...
      companies.map(async ({ token, name }) => {
        ...
        try {
          ...
          errors.push({
            source: this.sourceId,
            stage: "parse",
            message: `Greenhouse ${token}: schema parse failed â” ${err instanceof Error ? err.message : String(err)}`,
          });
          return;
        }
